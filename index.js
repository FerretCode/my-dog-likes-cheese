...
const JSONPushKey = async function (
  filepath,
  keyName,
  nestedKey = null,
  logging = true
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    if (nestedKey === null) {
      ...
      messageToPrint = `Key ${keyName} has been added to ${
        nestedKey.split(".")[nestedKey.split(".").length - 1]
      }`;
    }

    if (logging) console.log(messageToPrint);
...
const JSONPushValue = async function (
  filepath,
  key,
  value,
  secondValue,
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    if (nestedKey === null) {
      ...
      messageToPrint = `Value ${value} has been added to ${
        nestedKey.split(".")[nestedKey.split(".").length - 1]
      }`;
    }
...
const JSONDeleteValue = (
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    if (JSONParsedContent[key] && nestedKey === null) {
      ...
      messageToPrint = `Value ${value} has been removed from ${
        nestedKey.split(".")[nestedKey.split(".").length - 1]
      }`;
    } else if (nestedKey !== null && typeof nestedKey === "string" && !value) {
      ...
      messageToPrint = `Key ${key} has been removed from ${
        nestedKey.split(".")[nestedKey.split(".").length - 1]
      }`;
    }
